---
title: "Danish Kings"
author: "Andreas Blinksbjerg-Kristensen"
date: "2025-12-03"
...
library(tidyverse)
kings1 <- read.csv("data/danish_monarchs_full_estimates.csv")

kings2 <- read_csv("data/danish_monarchs_full_estimates.csv")

kings3 <- read.csv2("data/danish_monarchs_full_estimates.csv")

kings4 <- read_csv2( "data/danish_monarchs_full_estimates.csv")
```
Answer: 
1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object
Underscore indicates tidyverse
...
Using the command ncol on the different objects I can read in the console that kings have 5 columns.
...
kings <- read_csv2("data/danish_monarchs_full_estimates.csv")
...
kings %>%
  filter(!is.na(Reign_Start), !is.na(Reign_End)) %>%      
...
```{r}
# YOUR CODE
current_year <- as.numeric(format(Sys.Date(), "%Y")) # to include Frederik X

kings_clean <- kings %>%
  mutate(
    original_end = Reign_End,
    Reign_End = ifelse(is.na(Reign_End), current_year, Reign_End),
    duration  = Reign_End - Reign_Start,
    midyear   = Reign_Start + (Reign_End - Reign_Start) / 2)

ggplot(kings_clean, aes(x = midyear, y = duration)) +
  geom_point(size = 3) +
  geom_smooth(
    data = kings_clean %>% filter(!is.na(original_end)),
    method = "loess",
    se = TRUE
  ) +
  labs(
